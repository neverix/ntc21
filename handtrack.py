...
try:
    with mp_hands.Hands(
        ...
        subprocess.Popen(["python", "ht1.py"])
        fings = [0. for _ in range(8)]
        while True:
            cv2.waitKey(16)
            ...
            for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                # print(results.multi_handedness)
                for j, f in enumerate(["index_finger", "middle_finger", "ring_finger", "pinky"]):
                    fings[int(is_right) * 4 + j] = proc(dev(f"{f}"))
                '''
            ...
            dataset.append((time.time(),)+tuple(x for x in fings))
            '''
    ...
    import pandas as pd
    pd.DataFrame(dataset).to_csv("data.csv", index=0)